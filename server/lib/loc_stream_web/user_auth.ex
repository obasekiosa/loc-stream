defmodule LocStreamWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_loc_stream_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      LocStreamWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule LocStreamWeb.PageLive do
        use LocStreamWeb, :live_view

        on_mount {LocStreamWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{LocStreamWeb.UserAuth, :ensure_authenticated}] do
  ...
  def log_in_user_api(_conn, user, client_id, refresh_token \\ nil) do
    ...
    ## todo: delete previous refresh token if client_id matches
    if refresh_token do
